...
class VDLJNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new VDLJNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawDLJNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : VDLJNode {
        var curr : VDLJNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class VDividerLineJoiner {

    curr : VDLJNode = new VDLJNode(0)
    dir : number = 1 
